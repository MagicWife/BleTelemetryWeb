...
- `imu/` 基于该提交的 30 个原始文件，包含配置、源代码、预构建 Worker 和原有测试。2026-09-14 根据 BLE 量化精度放宽质量门控，并重新构建 Worker。该目录中的原始 README 描述的是参考项目；本项目接入方式以本文为准。

## 静止数据门控调整（2026-09-14）

这些阈值用于减轻静止误拒，尚未通过实际设备标注数据标定：

- 加速度幅值 MAD 轻微/强运动阈值：约 0.029/0.056 → 0.12/0.35 m/s²。
- 陀螺仪 RMS 轻微/强运动阈值：约 0.11/0.36 → 1.5/5 deg/s。协议一个量化步长为 0.01 rad/s，约 0.573 deg/s。
- 重复采样降级/拒绝比例：5%/50% → 95%/99.5%。完全恒定的数据仍拒绝，因为没有可用于估计的周期变化。
- 冲击跳变幅度增加 0.05 m/s² 下限，避免窗口 MAD 为零时把 0.01 m/s² 量化跳变误判为冲击；冲击占比阈值改为 15%/30%。
- 质量恢复确认从 3 个连续正常窗口减为 2 个；样本覆盖率与非有限数值阈值保持不变。
- 页面展示具体拒绝原因。放宽门控不保证一定有可用心率/呼吸率，候选结果仍需通过原算法有效性判断。
