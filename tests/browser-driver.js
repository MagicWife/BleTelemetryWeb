...
(function () {
  ...
  let index = 0;
  function bytesFor(i) {
    const bytes = new Uint8Array(42), view = new DataView(bytes.buffer), t = i / 50;
    ...
    view.setUint32(36, i * 20, true);
    view.setUint16(40, crc16Ccitt(bytes.subarray(0, 40)), true);
    return bytes;
  }
  const timer = window.setInterval(() => {
    // Multiple frames per Notify, split across an arbitrary byte boundary.
    const bytes = new Uint8Array(42 * 50);
    for (let i = 0; i < 50; i++) bytes.set(bytesFor(index++), i * 42);
