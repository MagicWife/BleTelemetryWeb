...
BLE 接收与页面渲染相互独立：所有有效帧均可被记录，实时数值、波形图和 3D 姿态只以 10 FPS 使用最新样本刷新。原始二进制解析仍在每次 Notify 到达时立即执行，不会因为降低界面刷新率而漏掉记录数据。
...
CSV 不再保存原始十六进制帧、协议版本、帧长、序号、MAC、运行时间或 CRC。连续记录时，浏览器可能询问是否允许当前网站自动下载多个文件，需要选择允许。
